...
class OWcutesv(OWBwBWidget):
    ...
    docker_image_name = "quay.io/biocontainers/cutesv"
    docker_image_tag = "2.1.1--pyhdfd78af_0"
    ...
    workDir=pset([])
    nThreads=pset(None)
    maxClusterBiasIns=pset(None)
    diffRatioMergingIns=pset(None)
    maxClusterBiasDel=pset(None)
    diffRatioMergingDel=pset(None)
    reportReadid=pset(False)
    genotype=pset(False)
    def __init__(self):
