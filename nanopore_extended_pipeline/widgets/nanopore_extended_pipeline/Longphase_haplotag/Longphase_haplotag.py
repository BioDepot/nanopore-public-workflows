...
class OWLongphase_haplotag(OWBwBWidget):
    name = "Longphase_haplotag"
    description = "Longphase haplotag"
    priority = 10
    ...
    threads=pset(None)
    svFile=pset([])
    outPrefix=pset("phased_bam")
    qualityThreshold=pset(None)
    def __init__(self):
        super().__init__(self.docker_image_name, self.docker_image_tag)
        with open(getJsonName(__file__,"Longphase_haplotag")) as f:
