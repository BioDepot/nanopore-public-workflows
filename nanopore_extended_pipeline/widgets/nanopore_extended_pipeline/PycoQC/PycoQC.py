...
class OWPycoQC(OWBwBWidget):
    name = "PycoQC"
    description = "PycoQC long read QC plots"
    priority = 1
    ...
    optionsChecked=pset({})
    inputFile=pset([])
    minPassQual=pset(None)
    minPassLen=pset(None)
    filterCalibration=pset(False)
    filterDuplicated=pset(False)
    minBarcodePercent=pset(None)
    reportTitle=pset(None)
    templateFile=pset(None)
    bamFile=pset([])
    verbose=pset(False)
    outputDir=pset(None)
    def __init__(self):
        super().__init__(self.docker_image_name, self.docker_image_tag)
        with open(getJsonName(__file__,"PycoQC")) as f:
